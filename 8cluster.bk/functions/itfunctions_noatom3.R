atom.select <- function(set, res, type) {
    if (is.null(set)) {
        set <- list(res,rep(type,length(res)))
    } else {
        set[[1]] <- c(set[[1]],res)
        set[[2]] <- c(set[[2]],rep(type,length(res)))
    }
    set
}
...
get.list <- function(i, atom) {
    ...
    sel <- i[[2]]
    if (ii > 1) {
        previous <- sum(atom[[1]][0:(ii - 1)])
    } else {
        previous <- 0
    }
    BB <- c("N","CA","C","O")
    if (sel == "ALL") { ind <- which(!(atom[[2]][ii,] %in% "")) 
    } else if (sel == "BB") { ind <- which(atom[[2]][ii,] %in% BB) 
    } else if (sel == "SC") { ind <- which(!(atom[[2]][ii,] %in% c(BB,""))) 
    } else { ind <- which(atom[[2]][ii,] %in% sel) }
    ind <- sort(c((3*ind-2),(3*ind-1),3*ind))
    ind + 3*previous
}
...
conditional.n.body.total.intercorrelation.lookup <- function(C, i, j, atom) {
    ...
    if (as.numeric(lookup[[m]][which(lookup[[m]][,1]==toString(k)),2])==0) {
        if (n > 1) {
            tmp <- conditional.n.body.total.intercorrelation.lookup(C, i[1:(n -1)], j, atom) - conditional.n.body.total.intercorrelation.lookup(C, i[1:(n -1)], append(i[n], j,), atom) + n.body.conditional.marginal(C, i[n], i[1:(n-1)], j, atom)
...
entropy.ave.terms.norm.lookup <- function(C, i, atom) {
    ...
    numb.terms <- sapply(1:length(i), choose, n=length(i))
    terms/(numb.terms*3*min(atom[[1]][i]))
}
...
entropy.ave.terms.total.intercorrelation.norm.lookup <- function(C, i, atom) {
    ...
    for (j in seq(1, length(i), 1) ) {
        ...
        sum <- 0
        for (k in 1:nrow(combinations)) {
            normalization <- (3*sum(atom[[1]][combinations[k,]]) - 1)
            if (j==length(i) & k==1) {
...
n.body.inter.corr <- function(C, i, atom) {
    info <- n.body.total.intercorrelation.lookup(C, i, atom, lookup.new=TRUE)
    sqrt(1 - exp((-2*info)/(3*sum(atom[[1]][unique(unlist(i))]) - 1)))
}
...
n.body.info.corr <- function(C, i, atom) {
    ...
    sqrt(1 - exp((-2*info)/(3*min(atom[[1]][unique(unlist(i))]))))
}
...
total.intercorrelation <- function(C, i, j, atom) {
    union_ij <- union(i, j)
    intersect_ij <- intersect(i, j)
    setdiff_i <- setdiff(i, intersect_ij)
    setdiff_j <- setdiff(j, intersect_ij)
    Lij <- NULL
    for (m in union_ij) {
        Lij <- c(Lij, get.list(m, atom))
    }
    sum(0.5*log(2*pi*exp(1)*diag(C[Lij, Lij]))) - entropy(C, union_ij, atom) - max(0, conditional.total.correlation.full.resolution(C, setdiff_i, setdiff_j, atom), na.rm = T) - max(0, conditional.total.correlation.full.resolution(C, setdiff_j, setdiff_i, atom), na.rm = T)
}
######HERE!!!!
total.intercorrelation.coefficient <- function(C, i, j, atom) {
    #    sqrt(1 - exp(-2*total.intercorrelation(C, i, j, atom)/(3*sum(atom[[1]][c(i,j)]) - 1)))
    
    BB <- c("N","CA","C","O")
    if (atom[[3]] == "") {
        ind1 <- which(!(atom[[2]][i,] %in% ""))
        ind2 <- which(!(atom[[2]][j,] %in% ""))
    }    else if (atom[[3]] == "BB") {
        part <- c(BB,"NOPROT")
        ind1 <- which(atom[[2]][i,] %in% part)
        ind2 <- which(atom[[2]][j,] %in% part)
    }    else if (atom[[3]] == "SC") {
        part <- c(BB,"")
        ind1 <- which(!(atom[[2]][i,] %in% part))
        ind2 <- which(!(atom[[2]][j,] %in% part))
    }    else {
        part <- c(atom[[3]],"NOPROT")
        ind1 <- which(atom[[2]][i,] %in% part)
        ind2 <- which(atom[[2]][j,] %in% part)
    }
    natoms <- length(c(ind1,ind2))
    #print(natoms)
    sqrt(1 - exp(-2*total.intercorrelation(C, i, j, atom)/(3*natoms - 1)))
}

total.intercorrelation.channel <- function(C, i, j, atom) {
    seq <- 1:length(atom[[1]])
    score <- sapply(seq, conditional.total.intercorrelation, c=C, i=i, j=j, atom=atom)
    total.intercorrelation(C, i, j, atom) - score    
}

calculate.average.total.intercorrelation.channel <- function(C, i, j, atom) {
    data <- NULL
    for (k in 1:length(C)) {
        data <- rbind(data, total.intercorrelation.channel(C[[k]], i, j, atom)) 
    }
    list(apply(data, 2, mean), apply(data, 2, sd))
}
