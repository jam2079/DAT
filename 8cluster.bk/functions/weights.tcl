...
mol new "$prot\_run$run\_selstructure.psf" waitfor all
mol off top
mol addfile "$prot\_run$run\_seltrajectory.dcd" type dcd first 0 last -1 step $step waitfor all
set mol top
set name "$prot\_run$run\_ifitbyres_stride_$step.dat"
...
set name "$prot\_run$run\_selweistructure.psf"
set test [file exists $name]
if { $test == 0 } {


	set fp [open "$prot\_run$run\_ifitbyres_stride_$step.dat" r]
