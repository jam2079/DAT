rmsdplot <- function(prot,run) {
    datfile <- paste("../../rmsd/data/",prot,"_run",run,"_rmsd.dat",sep="")
    pngfile <- paste("../../rmsd/plots/",prot,"_run",run,"_rmsd.png",sep="")
	...
	if (!file.exists(pngfile)) {	
        ...
        for (i in 1:length(TMx)) {
            datfile=paste("../../rmsd/data/",prot,"_run",run,"_rmsd_tm_",TMx[i],".dat",sep="")
            pngfile=paste("../../rmsd/plots/",prot,"_run",run,"_rmsd_tm_",TMx[i],".png",sep="")
	    if (file.exists(pngfile)) {next}
	    rmsdtm <- as.vector(as.matrix(read.table(datfile)))
            png(pngfile,width=1100,height=825,res=160)
            title=paste(prot,"run",run,"RMSD all TMs and TM",TMx[i])
            plot(frame,rmsd,"l",main=title,xlab="Frame", ylab="RMSD")
