...
set name "protein/$prot\_run$run\_weialitrajectory.dcd"
set test [file exists $name]
if { $test == 0 } {

	mol delete all
	mol new "protein/$prot\_run$run\_weistructure.psf" waitfor all
	mol off top
	mol addfile "protein/$prot\_run$run\_trajectory.dcd" waitfor all
