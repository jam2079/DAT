...
mol new "DAT_run$run\_selweistructure.psf" waitfor all
mol off top
mol addfile "DAT_run$run\_fixed_long.dcd" type dcd first [expr 100*$n] last [expr 100*$n+100] waitfor all
...
for {set i 0} {$i < [expr $nf - 1]} {incr i} {
	...
	if { $n == 0 } {
		if { $i == 0 } {
			file copy "dcd0.pdb" "DAT_run$run\_transfer.pdb"
		}
	}	
