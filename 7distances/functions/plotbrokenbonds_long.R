...
broken.bond <- function(r1,r2,run){
    a <- as.vector(as.matrix(read.table(paste("distances/data/distance_",r1,"_",r2,"_DAT_run",run,"_long.dat",sep=""))))
...
path <- "cluster/"
datfile<-paste(path,"data/numbrokenbonds_",prot,"_run",run,"_long.dat",sep="")
write.table(num,file=datfile,col.names=FALSE,row.names=FALSE)

pngfile<-paste(path,"plots/numbrokenbonds_",prot,"_run",runn,"_long.png",sep="")
