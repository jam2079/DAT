...
i <- i+1
datfile <- paste("data/distance_",prot,"_run",run,"_Na1_Na2.dat",sep="")
a <- read.table(datfile)
a <- as.vector(as.matrix(a))
a <- a[1:lim]
all <- cbind(all,a)
...
png(paste("barplot_distances_correlation_Na1_Na2_run_",run,"_frames _1_to_",lim,".png",sep=""),width=1100,height=825,res=160)
par(mar=c(4,3,3,3),las=2)
barplot(nadata,main=paste("R^2 between Na1-Na2 distance and S2 residues distances \n run",run,"frames 1 to",lim),ylim=c(0,0.6),names.arg=lab,cex.names=0.5)
dev.off()
}
#}