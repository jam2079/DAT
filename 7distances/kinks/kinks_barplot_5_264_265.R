...
i <- i+1
datfile <- paste("data/kink_",prot,"_","run",run,"_bend5_264.txt",sep="")
a1 <- read.table(datfile)
datfile <- paste("data/kink_",prot,"_","run",run,"_bend5_265.txt",sep="")
...
png(paste("barplot_distances_correlation_kink_5_264_265_run_",run,"_frames _1_to_",lim,".png",sep=""),width=1100,height=825,res=160)
par(mar=c(4,3,3,3),las=2)
barplot(nadata,main=paste("R^2 between TM 5 res 264 265 kink angle and S2 residues distances \n run",run,"frames 1 to",lim),ylim=c(0,0.6),names.arg=lab,cex.names=0.5)
dev.off()
}
}
#}
